/* Perceptual colour distance, and choosing the particle ramps with it — at
   runtime, against whatever is actually behind the particles.

   **Why this is not decided offline like every other colour here.** A
   particle is a thin moving line with nothing behind it but the water, so it
   owes the background more than anything else on the map does. The background
   is not one thing: it is two basemaps' worth of water *or*, when a scalar
   field is on, that field's colour scale painted opaque — 27 possibilities,
   and a reader changes which one is up without touching the particles.

   One fixed pair of ramps has to clear all 27 at once, and the arithmetic of
   that is unforgiving: measured, the fixed coral sat ΔE 3.1 from `cmo.matter`
   and 7.9 from `cmo.algae`. It was chosen against water, and a colormap the
   reader turns on afterwards was never part of the problem it solved.

   Solving one background at a time is a far easier problem, and the gate can
   still prove it because the 27 are all known offline: `scripts/test-contrast.mjs`
   runs this same search over every one of them and holds the answers to the
   same bars the fixed ramps were held to. What moved is *when* the choice is
   made, not whether it is checked.

   No Leaflet and no DOM, so it stays testable in Node and portable to a native
   port — the same rule `geo.ts`, `ramp.ts` and `tiles.ts` follow. It takes its
   bars as an argument rather than reading the palette, for the same reason.

   The maths is CIEDE2000, matching `scripts/lib/colour.mjs` so a runtime
   answer and a gate answer are the same number. WCAG contrast ratio is the
   obvious substitute and is wrong here: it compares luminance only, and would
   call the storm red and the glider magenta identical. CIE76 is the other easy
   one and understates differences in exactly the blue region ocean basemaps
   occupy. */
...
/* What a chosen ramp has to clear ---------------------------------------- */

export interface Bars {
  /** From the water or colour scale behind it. A particle has no casing and
      no shape to fall back on, so this is the one it cannot fail. */
  background: number;
  /** From every marker. Far lower, deliberately: a marker is a small filled
      dot with a dark outline, sitting still, and shape and stillness separate
      it from a drifting trail long before hue does. */
  marker: number;
  /** From the other velocity field, when both are drawn. Two sets of drifting
      lines have nothing but colour to tell them apart, so this is held at the
      background bar rather than the marker one. */
  apart: number;
}

/** Whether a ramp is fit to draw over this background.

    **The runtime and `scripts/test-contrast.mjs` both call this**, and that
    is the whole basis of the gate: the offline check is only meaningful if it
    applies the same test the map applies. A gate with its own copy of the
    rule proves something adjacent to what ships. */
export function admissible(
  ramp: string[],
  against: { background: string[]; markers: string[]; apartFrom?: string[] },
  bars: Bars
): boolean {
  if (clearance(ramp, against.background) < bars.background) return false;
  if (clearance(ramp, against.markers) < bars.marker) return false;
  const other = against.apartFrom ?? [];
  return !other.length || clearance(ramp, other) >= bars.apart;
}

/** The colours a reader may ask for, as exemplars.

    **Exported so the gate offers exactly what the picker offers.** These are
    the names `test-contrast` proves; a name added here and nowhere else is
    checked, and a name added to the UI instead of here is not, which is the
    asymmetry worth designing out.

    Black is absent on measurement rather than taste: across all 27
    backgrounds it clears the bars over none of them, since a dark ramp
    cannot separate from dark water and cannot separate from a pale colour
    scale either. Offering an option that is never available is worse than
    not offering it. */
export const NAMED_TINTS: readonly (readonly [string, string])[] = [
